...
class SplatPreprocessing(BasePreprocessing):
    def __init__(
        ...
        save_dir: str = "./data/processed/experiment",
    ...
    def compute_color_mean_std(
        self,
        train_database_path: str = "./data/processed/experiment/train_database.yaml",
    ):
...
def read_gaussians(filename:Path | str):
    ...
    sh_features = sh_features[mask]
    label = label[mask]
    instance_label = instance_label[mask]
    normals = get_normals(rotation, log_scaling).cpu().numpy()[mask]
    ...
    if foreground is not None:
        ...
        normals = normals[(foreground > 0).squeeze()]
